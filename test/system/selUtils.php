 ...
 */ 
  ...
  function verifyCaptchaExist($check)
  {
  		$this->assertEquals($check, $this->isElementPresent("xiCaptchaImage"));
  		$this->assertEquals($check, $this->isElementPresent("xiCaptchaValue"));
  }
  
  
  function fillCaptcha($correct=true, $value=false)
  {
  		if($value== false)
  		{
  			$value = "00000";
  			...
  			{
  				sleep(1);
  				$query = ' SELECT `value` FROM `#__xijc_captcha` '
  						.' ORDER BY `id` DESC '
  						.' LIMIT 1';
  				$db =& JFactory::getDBO();
  				$db->setQuery($query);
  	    ...
  	    $this->type("xiCaptchaValue", "$value");
  	    return;
  }
  
  
// para1=captcha shoud be shown or not
	// para2=how many times failure attempt can be made
	function processCaptcha($showCaptcha, $tryFailCount = 0)
	{
		if(!$showCaptcha)
			return;
		//this message come one time only
		$this->assertTrue($this->isTextPresent("Displaying captcha"));
		
		for($i=1 ; $i <= $tryFailCount ; $i++)
		{		
			
	    	$this->verifyCaptchaExist(true);
			$this->fillCaptcha(false);
			$this->click("ptypesavebtn");
			$this->waitPageLoad();

			//every fail attempt will see this msg
			$this->assertTrue($this->isTextPresent("Invalid captcha"));
		}
		...
		//now no captcha should exist
		$this->verifyCaptchaExist(false);
	}
